...
_fenix_setup_ssh() {
    ...
    _fenix_q "docker exec '$name' bash -c '
        ...
        mkdir -p /run/sshd
    '" 2>/dev/null
    # Copy box's host keys so all containers have same fingerprint (needs sudo for private keys)
    sudo docker cp /etc/ssh/ssh_host_ecdsa_key "$name:/etc/ssh/" 2>/dev/null
    sudo docker cp /etc/ssh/ssh_host_ecdsa_key.pub "$name:/etc/ssh/" 2>/dev/null
    sudo docker cp /etc/ssh/ssh_host_ed25519_key "$name:/etc/ssh/" 2>/dev/null
    sudo docker cp /etc/ssh/ssh_host_ed25519_key.pub "$name:/etc/ssh/" 2>/dev/null
    sudo docker cp /etc/ssh/ssh_host_rsa_key "$name:/etc/ssh/" 2>/dev/null
    sudo docker cp /etc/ssh/ssh_host_rsa_key.pub "$name:/etc/ssh/" 2>/dev/null
